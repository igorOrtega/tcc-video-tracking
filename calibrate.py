...
class CameraCalibration:
    ...
    def __init__(self, chessboard_points_per_row, chessboard_points_per_column, camera_parameters_save_dir, calibration_images_dir, chessboard_square_size, criteria):
    ...
    def main_loop(self):
        ...
        if(len(image_paths) < 30):
            ...
            objp = np.zeros((self.chessboard_points_per_row * self.chessboard_points_per_column, 3), np.float32)
            objp[:, :2] = np.mgrid[0:self.chessboard_points_per_row, 0:self.chessboard_points_per_column].T.reshape(-1, 2)*self.chessboard_square_size
            ...
            ret_val, cam_mtx, dist, _, _ = cv2.calibrateCamera(objpoints, imgpoints, gray.shape[::-1], None, None)
            ...
            if(not ret_val):
                ...
                print("Camera Matrix")
                print(cam_mtx)
                np.save(self.camera_parameters_save_dir + 'cam_mtx.npy', cam_mtx)
